// Función serverless (Vercel) — compra de un pack prepago de recargas vía MercadoPago.
// Crea la preferencia de pago; el webhook acredita las recargas cuando se aprueba.
//
...
// Catálogo de packs (debe coincidir con el frontend)
const PACKS = {
  pack5:  { credits: 5,  price: 8000 },
  pack10: { credits: 10, price: 15000 },
  pack20: { credits: 20, price: 28000 },
};
...
export default async function handler(req, res) {
  ...
  try {
    const { data: pay, error: pe } = await sb.from('payments')
      .insert({ user_id: user.id, amount_clp: pack.price, status: 'pending', kind: 'pack', credits: pack.credits })
      .select('id').single();
    if (pe) throw pe;

    const pref = {
      items: [{ title: `Pack ${pack.credits} recargas Kowen`, quantity: 1, unit_price: pack.price, currency_id: 'CLP' }],
      ...
      metadata: { user_id: user.id, payment_id: pay.id, kind: 'pack', credits: pack.credits },
    };
